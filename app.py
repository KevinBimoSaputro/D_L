...
def fetch_all_items():
    try:
        response = st_supabase.table("dinkum_items").select("*").execute()
...
if not df.empty and "sell_price" in df.columns:
    try:
        ...
        if not df_price.empty:
            ...
            top_price = int(top_item.get("sell_price", 0))
            col_stats2.metric(
                "Item dengan Harga Tertinggi",
                f"{top_name} — {top_price:,} Dinks",
            )
        else:
...
# -----------------------------------------------------------------------------
# Tabs: daftar item & form
# -----------------------------------------------------------------------------
tab_list, tab_form = st.tabs(["Daftar Item", "Form Item"])
...
with tab_list:
    ...
    if df.empty:
        ...
        col_filters = st.columns([3, 1, 1, 1])
        ...
        # Filter NPC (opsional)
        npc_options = ["Semua"]
        if "npc_name" in df.columns:
            existing_npcs = sorted(
                {
                    str(npc).strip()
                    for npc in df["npc_name"].dropna().unique()
                    if str(npc).strip()
                }
            )
            npc_options += existing_npcs
        selected_npc = col_filters[2].selectbox("Filter NPC", npc_options, index=0)
        ...
        selected_sort = col_filters[3].selectbox(
            "Urutkan berdasarkan",
            SORT_OPTIONS,
            index=0,
        )
        ...
        # Apply NPC filter
        if selected_npc != "Semua" and "npc_name" in filtered_df.columns:
            filtered_df = filtered_df[filtered_df["npc_name"] == selected_npc]

        # Sorting
        if selected_sort == "Nama (A-Z)":
            filtered_df = filtered_df.sort_values(
                "name", ascending=True, na_position="last"
            )
            ...
            selected_sort == "Terbaru (updated_at)"
            and "updated_at" in filtered_df.columns
        ):
            filtered_df = filtered_df.sort_values(
    ...
    if not df.empty:
        ...
        with copy_col2:
            if selected_for_copy:
                ...
                sell_price_val = (
                    int(row_copy["sell_price"]) if row_copy["sell_price"] else 0
                )
                info_str = (
                    f"Nama: {row_copy['name']}\n"
                    f"Harga: {sell_price_val:,} Dinks\n"
                    f"Kategori: {row_copy.get('category') or '-'}\n"
                    f"NPC: {row_copy.get('npc_name') or '-'}\n"
...
with tab_form:
    ...
    # Gunakan expander sebagai container untuk form tambah/update item
    with st.expander("Tambah / Update Item Dinkum", expanded=True):
        ...
        with form_col2:
            # NPC dengan dropdown + opsi nama baru
            npc_options_form = ["(Tulis nama NPC baru)"]
            if "npc_name" in existing_items_df.columns:
                existing_npcs_form = sorted(
                    {
                        str(npc).strip()
                        for npc in existing_items_df["npc_name"].dropna().unique()
                        if str(npc).strip()
                    }
                )
                npc_options_form += existing_npcs_form

            # Tentukan index default untuk update
            default_npc_index = 0
            if default_npc and default_npc in npc_options_form:
                default_npc_index = npc_options_form.index(default_npc)

            selected_npc_option = st.selectbox(
        ...
        if submitted:
            if not name_input.strip():
                ...
                payload = {
                    ...
                    "npc_name": npc_input.strip() if npc_input.strip() else None,
